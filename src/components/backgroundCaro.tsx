...
export default function BackgroundGameCaro() {
    const [file, setFile] = useState<any | null>(null);
    const [blob, setBlob] = useState<string | undefined>();
    const [success,setSuccess] = useState(false)
    const [loading,setLoading] = useState(false)

    let backgroundCaro = "backgroundCaro";
  ...
  const onSubmit = () => {
    ...
    setLoading(true);
    const fileRef = ref(storage,`/gameCaro/${backgroundCaro}`);
    ...
    return (
      <div>
        <p className="ml-7 mb-3 font-bold text-2xl">BACKGROUND GAME CARO SCREEN(9:16)</p>
